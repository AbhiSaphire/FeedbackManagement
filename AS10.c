...
struct response *front = NULL;
struct response *rear = NULL;
...
struct response *acceptDate(struct response *temp){
    ...
    if((temp->date[3]) == 1){
        if((temp->date[4]) > 2)
        {
            printf("\n\t\tDate entered is wrong3!!");
            acceptDate(temp);
        }
    }
    if((temp->date[0]) > 3){
        printf("\n\t\tDate entered is wrong4!!");
        acceptDate(temp);
    }
    if((temp->date[3]) > 1){
        printf("\n\t\tDate entered is wrong5!!");
        acceptDate(temp);
    }
    */
...
int main(){
    ...
    while(1){
        system("cls");
        header();
        printf("\n\t\tENTER YOUR CHOICE-\n\t\t1)CREATE\n\t\t2)DISPLAY\n\t\t3)INSERT AT POSITION\n\t\t4)DELETE\n\t\t5)DELETE AT POSITION\n\t\t6)SORT\n\t\t7)SEARCH\n\t\t8)MODIFY\n\t\t9)CREATE USING QUEUE (SEPERATED)\n\t\t10)DISPLAY QUEUE ELEMENTS\n\t\t11)DELETE FROM QUEUE\n\t\t12)DISPLAY TREE OF STARS\n\t\t13)EXIT: ");
        scanf("%d",&choice);
        switch (choice){
        case 1:
...
struct response *modify(struct response *listofresponse,char item[])
{
    struct response *p = listofresponse;
    char ans1,ans2;
    int chw=0;
    if(strcmp(p->title, item)==0){
        printf("\n\t\tITEM FOUND\n");
        ...
        do
        {
            ...
            switch (choice)
            {
                case 1:
                    ...
                    break;
                case 2:
                    printf("\n\t\tCURRENT NUMBER OF STARS GIVEN TO %s IS %d", p->title, p->stars);
    ...
    while(p->link!=NULL)
    {
        if(strcmp(p->link->title,item)==0)
        {
            ...
            do
            {
                ...
                {
                    case 1:
                        printf("\n\t\tCURRENT TITLE NAME IS %s", p->link->title);
                        printf("\n\t\tEnter new title name: ");
                        scanf(" %[^\n]s",(p->link->title));
                        break;
                    case 2:
                        printf("\n\t\tCURRENT NUMBER OF STARS GIVEN TO %s IS %d", p->link->title, p->link->stars);
                        printf("\n\t\tEnter new number of stars to %s: ", p->link->title);
                        scanf("%d",&(p->link->stars));
                        break;
                    case 3:
                        printf("\n\t\tCURRENT NAME IS%s ", p->link->name);
                        printf("\n\t\tEnter new name: ");
                        scanf(" %[^\n]s",(p->link->name));
                        break;
                    case 4:
                        printf("\n\t\tEnter new Feedback for %s: ", p->link->title);
                        scanf(" %[^\n]s",(p->link->response));
                        break;
                    default:    printf("\n\t\tWRONG CHOICE");
                }
...
void displayQ(){
    ...
    while(temp->link != NULL){
        printf("\n%20s%20s%20s\t%d%40s",front->title, front->name, front->date, front->stars, front->response);
        temp = temp->link;
    }
    printf("\n%20s%20s%20s\t%d%40s\n",front->title, front->name, front->date, front->stars, front->response);
}
